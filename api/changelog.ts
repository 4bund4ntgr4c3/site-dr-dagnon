...
export const CHANGELOG_ENTRIES: ChangelogEntry[] = [
  ...
  },
  {
    ...
    version: '4.1',
    label: { fr: '6 septembre 2026', en: '6 September 2026' },
    title: { fr: 'Audit SEO P0/P1, PageSpeed 100/100 Mobile, Vercel Analytics & Robots.txt RFC 9309', en: 'SEO P0/P1 Audit, Mobile PageSpeed 100/100, Vercel Analytics & RFC 9309 Robots.txt' },
    fr: [
      'Audit SEO & Indexabilité P0 — Élimination de la balise titre vide `<h3>«  »</h3>` sur la page d’accueil par l’ajout des clés de traduction bilingues de la tribune à la une co-signée avec la Pr. Rose Leke (`media.featuredOpEd`, `media.featuredQuote`, `media.featuredSummary`, `media.readOpEd`).',
      'Conformité Robots.txt RFC 9309 — Unification en un seul bloc `User-agent: *`, exclusion explicite des routes `/offline`, distinction stricte entre moteurs de recherche / grounding IA (`OAI-SearchBot`, `ChatGPT-User`, `PerplexityBot` autorisés avec `/llms.txt`) et scrapers d’entraînement brut bloqués (`GPTBot`, `ClaudeBot`, `Google-Extended`, `CCBot`, `Bytespider`, etc.).',
      ...
      'Performance Mobile PageSpeed 100/100 — Hero CSS-only sans JavaScript bloquant, compression WebP q60 de la photo portrait avec dimensions responsives, chargement différé des statistiques d’impact — LCP abaissé de 6,4 s à 412 ms sur mobile (+21 points d’indice de performance).',
      'Télémétrie Privacy-First & CSP — Intégration transparente de `@vercel/analytics` et `@vercel/speed-insights` pour le suivi Core Web Vitals en production sans cookie ni consentement intrusif, et ouverture ciblée de la CSP dans `vercel.json` pour Cloudflare Insights.',
      'Métadonnées Google Scholar — Intégration des balises Highwire (`citation_author`, `citation_title`, `citation_journal_title`, `citation_publication_date`, `citation_language`), normalisation du schéma éditorial `Article` et `Project` avec attribution `isBasedOn` de la source originelle.',
    ],
    en: [
      'SEO & Indexing P0 Audit — Resolved empty `<h3>«  »</h3>` heading bug on the home page by adding complete bilingual translation keys for the featured op-ed co-authored with Prof. Rose Leke (`media.featuredOpEd`, `media.featuredQuote`, `media.featuredSummary`, `media.readOpEd`).',
      'RFC 9309 Robots.txt Standardization — Consolidated into a single canonical `User-agent: *` block, explicitly disallowed `/offline` routes, strictly separated AI search/citation agents (`OAI-SearchBot`, `ChatGPT-User`, `PerplexityBot` allowed via `/llms.txt`) from restricted model training scrapers (`GPTBot`, `ClaudeBot`, `Google-Extended`, `CCBot`, `Bytespider`, etc.).',
      ...
      'Mobile PageSpeed 100/100 Performance — CSS-only hero removing motion render-blocking overhead, responsive WebP q60 portrait optimization, lazy impact stats — slashing mobile LCP from 6.4s to 412ms (+21 performance index points).',
      'Privacy-First Telemetry & CSP — Frictionless integration of `@vercel/analytics` and `@vercel/speed-insights` tracking live Core Web Vitals with zero cookies or invasive consent gates, updated CSP in `vercel.json` for Cloudflare Insights.',
  ...
  },
  {
    date: '2026-09-04',
    version: '4.0',
    label: { fr: '4 septembre 2026', en: '4 September 2026' },
    title: { fr: 'Hydratation SSR Intégrale, Cartes OG Automatiques & Audit Transversal de Production', en: 'Complete SSR Prerender Hydration, Dynamic OG Cards & Full Production Audit' },
    fr: [
      'Hydratation SSR Exhaustive — Pré-rendu complet côté serveur (SSR) du corps de page HTML avec hydratation fluide sans flash pour toutes les collections (projets, tribunes, médias, timeline).',
      'Génération Automatique des Cartes Open Graph — Automatisation de la génération des cartes d’aperçu pour toutes les tribunes et fiches projets (`scripts/gen-og.mjs`), garantissant un affichage riche sur les réseaux professionnels (LinkedIn, X, WhatsApp) sans appel réseau runtime.',
      ...
      'Audit Transversal de Production — Développement du banc d’audit automatisé (`scripts/audit-production.mjs`) vérifiant le statut HTTP, les en-têtes CSP et le rendu mobile de l’ensemble des 130 URLs.',
      'Alignement des Routes Anglophones — Normalisation des redirections Edge pour les routes anglaises, élimination des doubles barres obliques et gestion cohérente du trailing slash.',
    ],
    en: [
      'Exhaustive SSR Prerender Hydration — Full server-side pre-rendering of HTML page bodies with smooth, zero-flash hydration across all collections (projects, op-eds, media, timeline).',
      'Automated Open Graph Cards — Automated generation of preview cards for all op-eds and project dossiers (`scripts/gen-og.mjs`), ensuring rich display on LinkedIn, X, and WhatsApp without runtime external network calls.',
      ...
      'Cross-Cutting Production Audit — Development of the automated production audit suite (`scripts/audit-production.mjs`) inspecting HTTP status, CSP headers, and mobile viewport rendering across all 130 URLs.',
      'English Route Alignment — Strict normalization of Edge route redirects for English URLs, eliminating double slashes and unifying trailing slash behavior.',
    ],
  },
  {
    date: '2026-09-02',
    version: '3.9',
    label: { fr: '2 septembre 2026', en: '2 September 2026' },
    title: { fr: 'Référencement Sémantique Avancé, GEO/LLMs (/llms.txt), Sécurité RFC 9116 & Code-Splitting Performance', en: 'Advanced Semantic SEO, GEO/LLMs Discovery (/llms.txt), RFC 9116 Security & Performance Code-Splitting' },
    fr: [
      'Graphe Sémantique Enrichi — Ajout du schéma `VideoObject` sur toutes les vidéos et keynotes avec vignettes et dates ISO, balisage `speakable` pour les assistants vocaux sur les tribunes, flux RSS dédié en français (`dist/feed-fr.xml`), enrichissement du schéma `MedicalScholarlyArticle`.',
      'Découvrabilité IA & GEO — Déploiement de `/llms.txt` structuré avec index markdown de l’ensemble des sections et fiches d’impact, autorisation explicite des robots de recherche et citation IA dans robots.txt.',
      'Sécurité & Conformité RFC 9116 — Publication de `/.well-known/security.txt` et `/humans.txt`, intégration de Google Consent Mode v2, durcissement des en-têtes de sécurité HSTS et Permissions-Policy.',
      'Performance & Découpage de Code — Fractionnement asynchrone de la page Impact (164 ko -> 16 ko, -90%), chargement différé (lazy loading) des sections sous la ligne de flottaison sur la page d’accueil (-76% de JS initial), préconnexions analytiques et OpenSearch.',
      'Ergonomie Médias — Repositionnement du studio de conférences interactif en bas de page pour fluidifier la lecture des tribunes.',
    ],
    en: [
      'Enriched Semantic Graph — Integrated `VideoObject` structured schema across all media and keynotes with thumbnails and ISO dates, `speakable` metadata for voice assistants on op-eds, dedicated French RSS feed (`dist/feed-fr.xml`), enriched `MedicalScholarlyArticle` schema.',
      'AI & GEO Discoverability — Deployed structured `/llms.txt` markdown index linking all core sections and country dossiers, explicitly permitted AI search and citation crawlers in robots.txt.',
      'RFC 9116 Security & Compliance — Published standard `/.well-known/security.txt` and `/humans.txt`, integrated Google Consent Mode v2, hardened HSTS and Permissions-Policy security headers.',
      'Performance & Code-Splitting — Async code-splitting of the Impact page (164 kB -> 16 kB, -90%), deferred lazy loading for below-the-fold homepage sections (-76% initial JS), optimized analytics preconnections and OpenSearch.',
      'Media UX — Relocated interactive Conference Studio section to bottom of page to prioritize op-ed readability.',
    ],
  },
  {
    date: '2026-09-01',
