...
export function Achievements() {
  ...
  return (
    <section id="realisations" className="relative overflow-hidden bg-pine-900 py-24 lg:py-32">
      ...
      {showAwards && (
        <div
          className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto bg-pine-950/90 p-4 pt-20 pb-20 backdrop-blur-sm"
        ...
        >
          <div
            ref={modalRef}
            className="modal-scroll relative w-full max-w-3xl overflow-y-auto rounded-3xl border border-white/10 bg-pine-950 shadow-2xl"
            style={{ maxHeight: '80vh' } as React.CSSProperties}
            onClick={(e) => e.stopPropagation()}
          >
            {/* header */}
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-pine-950/95 px-8 py-5 backdrop-blur">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gold-500 text-pine-950">
                  <Award size={19} />
                </span>
              ...
              <button
                ...
                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-ivory/70 transition-colors hover:bg-white/10 hover:text-ivory"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-8">
              <div className="relative">
                {/* vertical line */}
                <div className="absolute bottom-4 left-[19px] top-4 w-px bg-gradient-to-b from-gold-500 via-gold-500/40 to-transparent" />

                <div className="space-y-8">
                  {AWARDS[lang].map((award, i) => (
                    <div key={i} className="relative flex gap-5">
                      {/* dot */}
                      <div className="relative z-10 mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-gold-500/60 bg-pine-950">
                        <span className="text-[11px] font-bold text-gold-400">{award.year.slice(2)}</span>
                      </div>
                      {/* content */}
                      <div className="flex-1 rounded-2xl border border-white/8 bg-white/5 p-5 transition-colors hover:border-gold-500/25 hover:bg-white/[0.07]">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="rounded-full bg-gold-500/15 px-2.5 py-0.5 text-[11px] font-bold text-gold-400">
                            {award.year}
                          </span>
                          <h3 className="font-display text-[15px] font-semibold text-ivory">
                            {award.title}
                          </h3>
                        </div>
                        <p className="mt-2 text-[13px] leading-relaxed text-pine-100/60">
                          {award.description}
                        </p>
                        {award.quote && (
                          <div className="mt-3 rounded-xl border border-gold-500/20 bg-gold-500/5 p-4">
                            <div className="flex items-start gap-2">
                              <Quote size={14} className="mt-0.5 shrink-0 text-gold-500/50" />
                              <p className="text-[12.5px] italic leading-relaxed text-gold-300/80">
                                {award.quote}
                              </p>
                            </div>
                          </div>
