    ...
    ...(indexable
      ? [
          `    <link rel="canonical" href="${escapeAttr(meta.url)}" />`,
           ...(meta.url === 'https://seynudedagnon.com/' || meta.url === 'https://seynudedagnon.com/fr'
            ? [`    <link rel="preload" as="image" href="/dr-seynude-dagnon.webp" imagesrcset="/dr-seynude-dagnon-400.webp 400w, /dr-seynude-dagnon.webp 694w" imagesizes="(max-width: 1024px) 368px, 400px" fetchpriority="high" type="image/webp" />`]
            : []),
          ...meta.alternates.map(
...
async function run() {
  ...
  /* Private starter shells — boot client app from an empty React root with
     localized meta, noindex, and no private content, canonical, or JSON-LD. */
  const PRIVATE_ROUTES = ['/admin', '/changelog', '/newsletter/preferences'];
  for (const lang of PRERENDER_LANGS) {
    for (const route of PRIVATE_ROUTES) {
      ...
      const html = (before + privateHead + after)
